...
class Listar extends Component {
	...
	shouldComponentUpdate(nextProps) {
		let condition = (
			nextProps.tiposAnalisis !== this.props.tiposAnalisis ||
		...
		return <tbody>
			{
				tiposAnalisis.map((tipo) => {
					return <tr key={tipo.id_tipoAnalisis}>
			            ...
			            <td>
			           		<Link to={`/dashboard/tipos-analisis/${tipo.id_tipoAnalisis}/parametros`}>
	...
	render() {
		...
		const { tiposAnalisis, cargando } = this.props.listar
